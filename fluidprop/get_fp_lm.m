...
function [w, n, rho, eta, c_sat, D_AB] = get_fp_lm (pdir, id_f, T)
  ...
  switch (id_f)
    case "WG141"
      ...
      n = n_PDMS = ri_PDMS_T (T, calib_w.fit_n_PDMS.c); # see a_ri_matching.m
      w = ri_matching_mf (n_PDMS, T, calib_w.fit_n_PT.c);
      rho = get_fp_tab (pdir, fname, pname="rho", T, w, ext);
      eta = get_fp_tab (pdir, fname, pname="eta", T, w, ext);
      c_sat = 3.6399; # see "a_oxygen_diffusivity_solubility.m"
## TODO f(T, w)      D_AB = get_fp_tab (pdir, fname, pname="D", T, w, ext);
## TODO f(T, w)      c_sat = get_fp_tab (pdir, fname, pname="c_sat", T, w, ext);
##      D_AB.corr ... # see "a_oxygen_diffusivity_solubility.m"
      ...
      D_AB.PLIF1 = corr_D_T_eta (eta_meas, T_meas, D_AB.PLIF1, eta, T);
      D_AB.PLIF2 = corr_D_T_eta (eta_meas, T_meas, D_AB.PLIF2, eta, T);
    case "WG139"
      w = 0.44; # TODO
##      n = get_ri_matching_tab (pdir, fname="glycerol-water", pname="ri", T, w=w, ext=[]);
      n = ri_matching_ri (w, T, fit_n_PT.c);
      rho = get_fp_tab (pdir, fname, pname="rho", T, w, ext);
      eta = get_fp_tab (pdir, fname, pname="eta", T, w, ext);
    ...
    case "WP141"
      ...
      n = n_PDMS = ri_PDMS_T (T, calib_w.fit_n_PDMS.c); # see a_ri_matching.m
      w = ri_matching_mf (n_PDMS, T, calib_w.fit_n_PD.c);
      rho = get_fp_tab (pdir, fname, pname="rho", T, w, ext);
      eta = get_fp_tab (pdir, fname, pname="eta", T, w, ext);
  ...
  # ensure refractive index matching condition w (n (T))
  if (length (T) > 1)
    rho = diag (rho);
    rho = rho (find (rho));
    eta = diag (eta);
    eta = eta (find (eta));
    D_AB.PLIF1 = diag (D_AB.PLIF1);
    D_AB.PLIF1 = D_AB.PLIF1 (find (D_AB.PLIF1));
    D_AB.PLIF2 = diag (D_AB.PLIF2);
    D_AB.PLIF2 = D_AB.PLIF2 (find (D_AB.PLIF2));
  endif
