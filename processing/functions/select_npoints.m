...
function [points, fh] = select_npoints (npoints, msh, scmap, wg, plotextra, style, msg)
  ...
  if (! isempty (plotextra))
    hold on
    plot3 (plotextra{1}(:,1), plotextra{1}(:,2), ones(1, numel (plotextra{1}(:,1))), "r", "linewidth", 2);
    legend ("map", plotextra{2}, "interpreter", "none");
  endif
  hold on;
  if (! isempty (wg))
    i = 1; # wall
    plot3 (wg{i}(:,1), wg{i}(:,2), ones(1, numel (wg{i}(:,2))), "k", "linewidth", 2);
    i = 2; # gas
    plot3 (wg{i}(:,1), wg{i}(:,2), ones(1, numel (wg{i}(:,2))), "k", "linewidth", 2);
  endif
  ...
  while (! ok)
    ...
    switch (style)
      ...
      otherwise
        plot3 (points(:,1), points(:,2), ones(numel(points(:,1)),1), "linewidth", 2, "r-x");
    endswitch
