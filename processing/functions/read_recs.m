...
function [recs] = read_recs (pdir, pp, recids, method)
  ...
  for i = 1 : numel (recids)
    ...
    id = recids{i};
    rec_path = pp.(id).data;
    dat_path = [pdir.data id_path(rec_path)];

    if ( ! isempty (rec_path) )

      switch (method)
        case "AVG" # Ic1 + u
          str_piv = [dat_path "AVG-PIV.*.000000.csv"]; # 000000 identifies first export index
          str_plif = [dat_path "PLIF.*.000000.tif"];
        case "DYN" # Ic + u sync time series recordings
          str_piv = [dat_path "APIV.*.000000.csv"];
          str_plif = [dat_path "PLIF_DYN.*.tif"];
        case "APIV" # "SPIV DYN"
          str_piv = [dat_path "APIV.*.csv"];
        case "APIV_STAT" # vector statistics of APIV maps with AVG PLIF recordings
          str_piv = [dat_path "APIV_STAT*.csv"];
  ##        str_piv = [dat_path "APIV_STAT_comp2.*.000000.csv"];
          str_plif = [dat_path "PLIF.*.000000.tif"];
        case "PLIF_DYN" # only PLIF time series recordings
          str_plif = [dat_path "PLIF_DYN.*.tif"];
        case "SL" # stream line images
          str_plif = [dat_path "SL.*.tif"];
  ##        str_plif = [dat_path "S_2.*.tif"];
        otherwise
