...
function [a, cp_nn] = erfc_profile_fit_test (sn, cp_n, sig, idx_r, dcds_idx, opt, testplots_fit)
...
##  fh = figure ();
  ...
  for i_p = 1 : ski_p : n_p
    ...
    if (any (i_p == round ([1:4] * 25 * n_p / 100)))
      printf ([num2str(round(i_p/n_p*100)) " / 100 :: " num2str(i_p) " of " num2str(n_p) " profiles :: delta_c = " num2str(median(1e3*cn_fit_delta_c(a(i_p-round(n_p*0.25)+1:i_p,:)))) " µm\n"]);
    endif
