...
function pdat = corr_xy_min_offsets (pdat, a_type, it_A, it_C, it_M, it_X, moff)
  ...
  for id_A = it_A
    for id_C = it_C
      for id_M = it_M
        for id_X = it_X
          Ic_1 = (pdat.c_dat{id_A, id_C, id_M, id_X}{1});
          Ic_2 = (pdat.c_dat{id_A, id_C, id_M, id_X}{2});
          Ic_3 = (pdat.c_dat{id_A, id_C, id_M, id_X}{3});
          c_h_g = (pdat.c_h{id_A, id_C, id_M, id_X}).gas;
##          sf = get_sf ((pdat.c_msh(id_A, id_C, id_M, id_X){1}));
          coord_y = pdat.c_msh(id_A, id_C, id_M, id_X){1}{2}(:,1);
...
##            xoffs = -75:1:75;
          ...
          if ( ! (moff(id_X,1)==0) )
            xoffset = moff(id_X,1); # systematic shift of Ic1 relative to Ic0 & Ic
          endif
          xoffset
          opt_val = [];
          yoffs = -12:1:12;
...
##          pdat.c_dat{id_A,id_C,id_M,id_X}{1} = Ic_1;
##          pdat.c_dat{id_A,id_C,id_M,id_X}{2} = Ic_2;
          pdat.c_dat{id_A,id_C,id_M,id_X}{3} = Ic_3;
        endfor
      endfor
    endfor
  endfor
