...
function [delta_fit, cp_nn, cn0, p_c_fit, p_sc, cn_ds_0_fit] = erfc_profile_fit (snp, cp_n, sf_p, snp_ioff, sig, idx_r, dcds_idx, testplots_fit)
  ...
  for i = 1:ski_p:size(cp_n,1)
    ...
    ## find fit parameters
    [p_c_fit{i}, ~] = nonlin_curvefit (@(p, sn) fitfn_cn_diff(p, sn, p_sc), init, [snp(fit_idx(i,1):fit_idx(i,2))]', [cp_n(i,fit_idx(i,1):fit_idx(i,2))]', settings);
    cn0(i) = p_c_fit{i}(2);
    ...
    cp_nn(i,:) = cp_n(i,:);
    cp_nn(i,1:idx_max_dcds+1) = fitfn_cn_diff (p_c_fit{i}, snp(1:idx_max_dcds+1), p_sc);
    ...
    if testplots_fit
      ...
      plot (snp, fitfn_cn_diff(p_c_fit{i}, snp, p_sc), "g")
      plot ([0 0], [0 1.5], "k")
