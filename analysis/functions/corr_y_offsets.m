...
function pdat = corr_y_offsets (pdat, it_A, it_C, it_M, it_X)
    ...
    for id_C = it_C
      for id_M = it_M
        for id_X = it_X
          ...
          xpos = 0;
          [~, xc] = min (abs (pdat.c_msh{id_A, id_C, id_M, id_X}{1}(1,:) - xpos));
          [~, yc] = min (abs (pdat.c_msh{id_A, id_C, id_M, id_X}{2}(:,1) - pdat.c_h{id_A, id_C, id_M, id_X}.wall(xc)));
          ## mean area
          ys = 50; xs = 100;
  ...
  ##        figure(); hold on; plot(opt1); plot(opt2); plot(opt3)
