...
## read data for one given DAQ_log file
##
...
function [logs] = get_log_DAQ (path_to_logs)
  ...
  logs.time = t_sec;
  logs.mass_flow_rate = MFR;
  logs.temperature_inlet_gas    = TEMP(:,1);
  logs.temperature_inlet_liquid = TEMP(:,2);
  logs.temperature_box          = TEMP(:,3);
endfunction
