  ...
  switch ndims
    ...
    case 1
      ...
      if (! iscell (dat{it_X(1)}))
        ...
        for i_X = it_X
          for j = 1 : nmaps
            for i_col = 1 : columns (dat{i_X}{j})
              dat_st{i_X,j}(:,i_col) = interp1 (msh_xs{i_X}{1}(1,:), dat{i_X}{j}(:,i_col), msh_st_sec{i_X}{1}(1,:), "nearest", "extrap");
            endfor
          endfor
        endfor
    ...
    case 2
      ...
      if ( (nmaps == 1) && (! iscell(dat{it_X(1)})) && (numel (dat{it_X(1)}) == numel (msh_xs{it_X(1)}{1})) )
      ...
      elseif ( (nmaps == 1) && (! iscell(dat{it_X(1)})) ) # to stitch several colums of array along x
