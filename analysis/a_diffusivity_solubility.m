...
eta_PT_W = get_fp_tab (pdir, fname="glycerol-water", pname="eta", T_get=TK(3), mf_get=mf, ext=[]);
eta_PD_W = get_fp_tab (pdir, fname="propylene glycol-water", pname="eta", T_get=TK(3), mf_get=mf, ext=[]);
## density in kg / m^3 @ 20°C 22,5°C 25°C 30°C
...
## O2 ideal gas
rho_O2 = 101325 ./ (const_ug ./ mm_O2 .* TK(3));
...
D_PT_SE = f_D_SE_slip (T=TK(3), r_O2, [eta_W(3), eta_PT_W(2:end)]');
...
D_Am_PD_WC = f_DAB_WC (K_SI, phi_B=1, mm_B=phi_mm_PDW', eta_B=eta_PD_W', mv_A=mv_solute, T=TK(3));
## PT:
x_PT = fp_mx_mf (mf, mm_PT, mm_PTW_solvent);
## with mixing rule:
phi_mm_PTW = x_PT*phi_PT*mm_PT + (1-x_PT)*phi_W*mm_W;
D_Am_PT_WC = f_DAB_WC (K_SI, phi_B=1, mm_B=phi_mm_PTW', eta_B=eta_PT_W', mv_A=mv_solute, T=TK(3));
...
##D_WPT_S = f_DAB_S (Ls=L_PT, L=L_W, mm_PT, eta_PT_W(end), fp_mv_bp_TC(mv_crit_W), T=TK(3))
...
[mf_SJG_PT, ~, rho_PT_ri_match, ~, ~, D_AB] = get_fp_lm (pdir, "WG141", 298.15); # from RI-matching
D_SJG_PT_1 = D_AB.PLIF1;
D_SJG_PT_2 = D_AB.PLIF2;
##
[mf_SJG_PD, ~, rho_PD_ri_match, ~, ~, D_AB] = get_fp_lm (pdir, "WP141", 298.15); # from RI-matching
...
vf_PD_Y = fp_mf_mx (ds.data{2}, mm_PD/rho_PD(3), mm_W/rho_W(3));
...
##c_sat_Benson_fit = 1e3 * rho_W(3) .* mf_sat_Benson_fit # mg/l
...
##c_sat_iupac_fit = 1e3 * rho_W(3) .* mf_sat_iupac_fit # mg/l
