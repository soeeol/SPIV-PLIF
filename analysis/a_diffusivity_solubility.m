...
eta_W = get_fp_tab (pdir, fname="water", pname="eta", T_get=TK, w_get=1, ext=[]);
eta_PT_W = get_fp_tab (pdir, fname="glycerol-water", pname="eta", T_get=T_ref, w_get=w, ext=[]);
eta_PD_W = get_fp_tab (pdir, fname="propylene glycol-water", pname="eta", T_get=T_ref, w_get=w, ext=[]);
## density in kg / m^3
rho_W = rho_PT_W_model (0, TK);
rho_PT = rho_PT_W_model (1, TK);
rho_PD = get_fp_tab (pdir, fname="propylene glycol-water", pname="rho", T_get=TK, w_get=1, ext=[]);
...
## PT:
x_PT = fp_mx_mf (w, mm_PT, mm_PTW_solvent);
## with mixing rule:
...
mre_PT = 0.5;
x_2 = fp_mx_mf (1-w, mm_W, mm_PTW_solvent);
x_3 = fp_mx_mf (w, mm_PT, mm_PTW_solvent);
...
mre_PD = 0.66;
x_2 = fp_mx_mf (1-w, mm_W, mm_PDW_solvent);
x_3 = fp_mx_mf (w, mm_PD, mm_PDW_solvent);
eta_2 = eta_W(TK==T_ref)
eta_3 = eta_PD_W(end)
eta_m = eta_PD_W;
...
## NogamiH1962, polarographic, T = 295.65 K
## Nogami and Kato (1962), doi: 10.1248/yakushi1947.82.1_120.
## PT
ds = get_fp_dataset (fp, {"PT_W"}, {"D_AB"}, {"NogamiH1962"});
T_NK = ds.data{1};
vf_PT_NK = ds.data{2}; # vol fraction glycerol
D_PT_NK = ds.data{3}; # diffusivity m^2/s
ds = get_fp_dataset (fp, {"PT_W"}, {"c_sat"}, {"NogamiH1962"});
csat_PT_NK = ds.data{3}; # mol/l
##phi_PT_NK = [2.6 2.57 2.45 2.3 2.06]; # association factor Wilke Chang
## conversion
w_PT_NK = vf_PT_NK.*rho_PT(TK==T_NK) ./ (vf_PT_NK.*rho_PT(TK==T_NK) + (1-vf_PT_NK).*rho_W(TK==T_NK));
eta_PT_NK = get_fp_tab (pdir, fname="glycerol-water", pname="eta", T_get=T_NK, w_get=w_PT_NK, ext=[]);
eta_PT_NK_T25 = get_fp_tab (pdir, fname="glycerol-water", pname="eta", T_get=T_ref, w_get=w_PT_NK, ext=[]);
...
##
## PD
ds = get_fp_dataset (fp, {"PD_W"}, {"D_AB"}, {"NogamiH1962"});
vf_PD_NK = ds.data{2}; # vol fraction glycerol
D_PD_NK = ds.data{3}; # diffusivity m^2/s
ds = get_fp_dataset (fp, {"PD_W"}, {"c_sat"}, {"NogamiH1962"});
csat_PD_NK = ds.data{3}; # mol/l
## conversion
w_PD_NK = vf_PD_NK.*rho_PD(TK==T_NK) ./ (vf_PD_NK.*rho_PD(TK==T_NK) + (1-vf_PD_NK).*rho_W(TK==T_NK));
eta_PD_NK = get_fp_tab (pdir, fname="propylene glycol-water", pname="eta", T_get=T_NK, w_get=w_PD_NK, ext=[]);
eta_PD_NK_T25 = get_fp_tab (pdir, fname="propylene glycol-water", pname="eta", T_get=T_ref, w_get=w_PD_NK, ext=[]);
...
## JordanJ1956, polarographic, T = 25°C
## Jordan et al. (1956), doi: 10.1021/ja01594a015.
#
ds = get_fp_dataset (fp, {"PT_W"}, {"D_AB"}, {"JordanJ1956"});
w_Jetal = ds.data{2};
D_Jetal = ds.data{3};

ds = get_fp_dataset (fp, {"PT_W"}, {"c_sat"}, {"JordanJ1956"});
c_org_Jetal = ds.data{3};
c_mg_Jetal = c_org_Jetal * mm_O2 * p_O2/p_total * 1e3; # mg / l

ds = get_fp_dataset (fp, {"PT_W"}, {"eta"}, {"JordanJ1956"});
eta_Jetal = ds.data{3};

## checking the slope of D vs. viscosity
mre_deta_NK = 1 ./ (eta_PD_NK_T25).^mre_PD;
mre_deta_NK_PT = 1 ./ (eta_PT_NK_T25).^0.9;
mre_deta_Jetal = 1 ./ (eta_Jetal).^mre_PT;
fh = figure (); hold on;
loglog (eta_PD_NK_T25, D_PD_NK_T25, "d;PD exp. NogamiH1962;")
loglog (eta_PT_NK_T25, D_PT_NK_T25, "x;PT exp. NogamiH1962;")
loglog (eta_Jetal, D_Jetal, "s;PT exp. JordanJ1956;")
...
loglog (eta_Jetal, (D_Jetal(2)+D_Jetal(1))/2.*mre_deta_Jetal./mre_deta_Jetal(1), ";slope mre=0.5;")
...
[w_SJG_PT, ~, rho_PT_ri_match, ~, ~, D_AB] = get_fp_lm (pdir, "WG141", T_ref); # from RI-matching
D_SJG_PT_1 = D_AB.PLIF1;
D_SJG_PT_2 = D_AB.PLIF2;
##
[w_SJG_PD, ~, rho_PD_ri_match, ~, ~, D_AB] = get_fp_lm (pdir, "WP141", T_ref); # from RI-matching
...
eta_W_K = get_fp_tab (pdir, fname="water", pname="eta",T_get=T_K, w_get=1, ext=[]);
...
w_PT_SJG = [0 0.2915	0.4369 0.5109 0.5880 0.6994 0.7991 0.9097];
c_mg_PT_SJG_min = [8.025 5.9 4.85 4.15 3.42 2.93 2.5 2.12];
w_PD_SJG = [0 0.2 0.4 0.725 0.85 0.975];
c_mg_PD_SJG_min = [8.2 7 6 6.5 7.15 8.5];

## KutscheI1984etal
## Kutsche et al. (1984), doi: 10.1021/je00037a018.
w_Ketal = 1e-2 * [1.31 2.52 3.74 6.16 9.18 12.21 15.23 18.26]; # glycerol
...
w_Retal = fp_mf_mc (rho_W(4), rho_PT(4), m_org_Retal); # mass fraction in g/g
...
## solubility comparison for JordanJ1956
...
w_PD_Y = fp_mf_mx (ds.data{2}, mm_PD, mm_W);
...
c_mg_Y = 1e3 * ds.data{3} * w_O2 * rho_ideal;
...
w_sat_Benson_fit = fp_mf_mx ((p_O2/p_total) ./ f_k_x_Benson(T_fit), mm_O2, mm_W); # g/g
c_sat_Benson_fit = 1e3 * fp_mc_mf (rho_PT_W_model(0,T_fit), rho_O2, w_sat_Benson_fit); # mg/l
...
w_O2_iupac = fp_mf_mx (mx=(p_O2 ./ H_iupac), mm_A=mm_O2, mm_B=mm_W); # g/g
c_sat_iupac = 1e3 * fp_mc_mf (rho_PT_W_model(0,T_iupac), rho_O2, w_O2_iupac); # mg/l
...
w_O2_iupac_fit = fp_mf_mx (p_O2 ./ f_k_x(T_fit), mm_O2, mm_W); # g/g
c_sat_iupac_fit = 1e3 * fp_mc_mf (rho_PT_W_model(0,T_fit), rho_O2, w_O2_iupac_fit); # mg/l
...
csat_Jetal_corr = f_c_mg_Retal (csat_W(TK==T_ref), Kn_PT(TK==T_ref), fp_mc_mf(rho_W(TK==T_ref), rho_PT(TK==T_ref), w_Jetal));
Dcorr_Jetal = 1 .* (csat_Jetal_corr ./ c_mg_Jetal).^2;
##
csat_NK_PT_corr = f_c_mg_Retal (csat_W(TK==T_NK), Kn_PT(TK==T_NK), fp_mc_mf(rho_W(TK==T_NK), rho_PT(TK==T_NK), w_PT_NK));
Dcorr_NK_PT = 1 .* (csat_NK_PT_corr ./ csat_PT_NK ).^2;
csat_NK_PD_corr = f_c_mg_Retal (csat_W(TK==T_NK), Kn_PD(TK==T_NK), fp_mc_mf(rho_W(TK==T_NK), rho_PD(TK==T_NK), w_PD_NK));
...
c_sat_PT_RI_match = f_c_mg_Retal(csat_W(TK==T_ref), Kn_PT(TK==T_ref), fp_mc_mf(rho_W(TK==T_ref), rho_PT(TK==T_ref), w_SJG_PT))
