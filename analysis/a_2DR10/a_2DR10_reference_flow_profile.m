...
if 1
  ...
  switch (ap.pos_ref_profile)
    case {"downstream"}
      delta_u_ref(2) = delta_u_ref(2) - 0.005;
      delta_u_ref(3) = delta_u_ref(3) - 0.01;
      delta_u_ref(4) = delta_u_ref(4) - 0.01;
      y_u{1} = y{1} - 0.005;
      y_u{2} = y{2} - 0.005;
      y_u{3} = y{3} + 0.01;
      y_u{4} = y{4} + 0.005;
    case {"full"}
      delta_u_ref(1) = delta_u_ref(1) + 0.005;
      delta_u_ref(2) = delta_u_ref(2) - 0.005;
      delta_u_ref(3) = delta_u_ref(3) - 0.005;
      delta_u_ref(4) = delta_u_ref(4) - 0.01;
      y_u{1} = y{1} - 0.005;
      y_u{3} = y{3} + 0.005;
      y_u{4} = y{4} + 0.005;
  ...
  switch (ap.pos_ref_profile)
    case {"downstream"}
      mfr_meas(1) = mfr_meas(1) * 0.98; # integral overpredicts mfr_meas close to wall
      mfr_meas(2) = mfr_meas(2) * 0.95;
      mfr_meas(3) = mfr_meas(3) * 0.95;
      mfr_meas(4) = mfr_meas(4) * 0.95;
    case {"full"}
